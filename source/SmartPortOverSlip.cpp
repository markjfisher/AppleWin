...
SmartPortOverSlip::SmartPortOverSlip(const UINT slot) : Card(CT_SmartPortOverSlip, slot)
{
  ...
  {
    throw std::runtime_error("There is an existing slot active for SP over SLIP. You can only have "
                             "1 of these cards active.");
  }
  active_instances++;
...
void SmartPortOverSlip::handle_write()
{
  ...
  // Fix the stack so the RTS in the firmware returns to the instruction after
  // the data
  rts_location += 3;
  ...
  // convert the unit_number given to the id of the target connection, as we may
...
void SmartPortOverSlip::read_block(const BYTE unit_number, Connection *connection, const WORD sp_payload_loc, const WORD params_loc)
{
  ...
  // Assume that (cmd_list_loc + 4 == params_loc) holds 3 bytes for the block
  // number. If it's in the payload, this is wrong and will have to be fixed.
  request.set_block_number_from_ptr(mem, params_loc);
  auto response = Requestor::send_request(request, connection);

  handle_response<ReadBlockResponse>(std::move(response),
                                     [this, sp_payload_loc](const ReadBlockResponse *rbr)
                                     {
...
void SmartPortOverSlip::write_block(const BYTE unit_number, Connection *connection, const WORD sp_payload_loc, const WORD params_loc)
{
  ...
  // Assume that (cmd_list_loc + 4 == params_loc) holds 3 bytes for the block
  // number. The payload contains the data to write
  request.set_block_number_from_ptr(mem, params_loc);
...
void SmartPortOverSlip::read(const BYTE unit_number, Connection *connection, const WORD sp_payload_loc, const WORD params_loc)
{
  ...
  request.set_address_from_ptr(mem, params_loc + 2); // move along by byte_count size. would be better to
                                                     // get its size rather than hard code it here.
  auto response = Requestor::send_request(request, connection);

  handle_response<ReadResponse>(std::move(response),
                                [sp_payload_loc](const ReadResponse *rr)
                                {
...
void SmartPortOverSlip::status(const BYTE unit_number, Connection *connection, const WORD sp_payload_loc, const BYTE status_code)
{
  ...
  auto response = Requestor::send_request(request, connection);
  handle_response<StatusResponse>(std::move(response),
                                  [sp_payload_loc](const StatusResponse *sr)
                                  {
...
void SmartPortOverSlip::InitializeIO(LPBYTE pCxRomPeripheral)
{
  ...
  // Set location in firmware that need to know the slot number for
  // reading/writing to the card
  ...
  // Modify the destination memory to hold the slot information needed by the
  // firmware. The pData memory is R/O, and we get an access violation writing
  // to it, so alter the destination instead.
