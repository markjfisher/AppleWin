...
public:
  ...
  template <typename T> void handle_simple_response(const std::unique_ptr<Response> response)
  {
  ...
  template <typename T, typename Func> void handle_response(const std::unique_ptr<Response> response, Func call_back)
  {
    auto specific_response = dynamic_cast<T *>(response.get());
